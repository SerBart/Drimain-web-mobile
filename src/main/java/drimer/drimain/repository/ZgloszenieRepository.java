...
public interface ZgloszenieRepository extends JpaRepository<Zgloszenie, Long> {
    ...
    @Query("SELECT z FROM Zgloszenie z WHERE z.dzial.id = :dzialId AND (:status IS NULL OR z.status = :status)")
    Page<Zgloszenie> findByDzialIdAndOptionalStatus(@Param("dzialId") Long dzialId, 
                                                    @Param("status") ZgloszenieStatus status, 
                                                    Pageable pageable);
    
    @Query("SELECT z FROM Zgloszenie z WHERE (:status IS NULL OR z.status = :status)")
    Page<Zgloszenie> findByOptionalStatus(@Param("status") ZgloszenieStatus status, 
                                         Pageable pageable);
}